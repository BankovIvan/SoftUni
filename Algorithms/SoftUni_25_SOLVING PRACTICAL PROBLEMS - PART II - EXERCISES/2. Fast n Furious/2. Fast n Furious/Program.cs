...
class Program
{
    ...
    private static void CheckSpeeding(Dictionary<string, List<Edge>> graph, KeyValuePair<string, List<Task>> task)
    {
        int i, j;
        ...
        for(i = 1; i < task.Value.Count; i++)
        {
            for(j = 0; j < i; j++)
            {
                if (task.Value[j].destination == task.Value[i].destination)
                {
                    continue;
                }

                if (CheckSpeeding(graph, task.Value[j], task.Value[i]))
                {
                    result.Add(task.Key);
                    return;
                }
    ...
    private static bool CheckSpeeding(Dictionary<string, List<Edge>> graph, Task start, Task end)
    {
        ...
        {
            arrival.Add(item.Key, double.MaxValue);
