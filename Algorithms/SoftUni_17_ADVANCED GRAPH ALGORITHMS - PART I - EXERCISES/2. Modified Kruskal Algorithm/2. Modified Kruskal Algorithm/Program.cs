...
class Program
{
    ...
    {
        int nNodes = 0, nEdges = 0, parentFrom, parentTo;
        double total = 0.0;
        List<Edge> edges;
        Dictionary<int, Vertex> graph;

        edges = GraphHelper.ReadEdges(ref nNodes, ref nEdges);

        edges.Sort();

        graph = GraphHelper.CreateEmptyGraph(edges, nNodes);

        foreach (var edge in edges)
        {
            parentFrom = FindRoot(graph, edge.nodeFrom);
            parentTo = FindRoot(graph, edge.nodeTo);

            if(parentFrom != parentTo)
            {
                graph[parentTo].SetParent(parentFrom);
                //No Need
                //graph[edgeFromID].AddChild(edge);

                //JUDGE wants edges printed reversed if grandParent[nodeFrom] > grandParent[nodeTo]...
                edge.SetVisited(true);
                total += edge.cost.Value;
            }
        }

        Console.WriteLine("Minimum spanning forest weight: {0}", (int)total);
        //GraphHelper.PrintEdgesSortedByParent(graph, edges, true);

        return;

    }

    private static int FindRoot(Dictionary<int, Vertex> graph, int nodeFrom)
    {
...
class GraphHelper
{
    ...
    public static Dictionary<int, Vertex> ReadGraph(ref int nNodes, ref int nEdges)
    {
