...
class Program
{
    ...
    {
        string s;
        int nNodes = 0, nEdges = 0, start = 0, finish = 0, currentNode;
        double nextElementCost;
        Dictionary<int, Vertex> graph;
        Queue<int> nodesQueue = new Queue<int>();

        graph = GraphHelper.ReadGraph(ref nNodes, ref nEdges, ref start, ref finish);

        currentNode = start;
        nodesQueue.Enqueue(currentNode);
        graph[currentNode].SetValue(1.0);
        graph[currentNode].SetParent(start);
        //graph[currentNode].SetVisited(true);
        ...
        while (nodesQueue.Count > 0)
        {
            ...
            {
                nextElementCost = graph[currentNode].cost.Value * edge.cost.Value;

                if (nextElementCost > graph[edge.nodeTo].cost.Value)
                {
                    graph[edge.nodeTo].SetValue(nextElementCost);
                    graph[edge.nodeTo].SetParent(currentNode);

                    nodesQueue.Enqueue(edge.nodeTo);
                    //graph[edge.nodeTo].SetVisited(true);
                }
            }

        }

        Console.WriteLine("Most reliable path reliability: {0:F2}%", graph[finish].cost.Value * 100.0);

        s = finish.ToString();
        while (graph[finish].parent.Value != finish)
        {
            s = graph[finish].parent.Value.ToString() + " -> " + s;
            finish = graph[finish].parent.Value;
        }
